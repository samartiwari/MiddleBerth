    ...
    public Mail write(NotificationEvent event, String to) {
        return switch (event.type()) {
            case TICKET_CONFIRMED -> new Mail(to,
                    ...
                    Happy journey.""".formatted(event.berth(), event.coachClass(), event.trainNumber(),
                            event.travelDate(), event.requestId()));
            ...
            case BOOKING_CANCELLED -> new Mail(to,
                    ...
                    Bank refunds usually take 5 to 7 working days.""".formatted(event.requestId(),
                            event.trainNumber(), event.travelDate()));
        };
    }
}
